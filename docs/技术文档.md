...
KafkaConsumerService.consumeContentReview(itemId)
  ...
  │
  ├─ 3. Kafka: send("content-change", body=ContentChangeMessage{itemId, eventType=UPDATE})
  │     → 通知 Canal 缓存同步管道：内容已审核通过，可以写入 Redis 缓存
  │
  └─ 4. Kafka: send("feed-fanout-request", key=creatorId, body=FanoutMessage{itemId,creatorId,publishTime})
        → key=creatorId，保证同一创作者的消息落到同一分区、有序处理

[阶段二] Canal 缓存同步：content-change topic
  │
  ▼
ContentCacheConsumer.onContentChange(message)   ← 监听内容变更事件
  │
  ├─ INSERT / UPDATE(status=1，审核通过)：
  │     ├─ 1. MySQL: SELECT * FROM item_info WHERE item_id=? → 取完整内容
  │     ├─ 2. Redis: HSET content:{itemId} → 写入内容详情 Hash（首页展示字段）
  │     └─ 3. Redis: ZADD outbox:{creatorId} publishTimeMs contentId → 写入发件箱 ZSET
  │
  └─ UPDATE(status≠1，下架) / DELETE：
        ├─ 1. Redis: DEL content:{itemId} → 清除内容缓存
        └─ 2. Redis: ZREM outbox:{creatorId} contentId → 从发件箱移除
  │
  ▼
  （注：本地未部署 Canal，此管道在本地通过 KafkaConsumerService 直接
    send("content-change") 模拟；生产环境可切换为 Canal 监听 MySQL binlog）

[阶段三] Fanout 推送：feed-fanout-request topic（3 个线程并行消费）
  │
  ▼
...
| Canal 缓存消费者 | service/ContentCacheConsumer.java |
| Canal 缓存服务 | service/ContentCacheService.java |
| Kafka 容器配置 | config/KafkaConfig.java |
| 消息体定义 | entity/FanoutMessage.java, entity/ContentChangeMessage.java |
| Redis Feed 工具 | utils/RedisFeedHelper.java |

**设计决策**：
- **三阶段异步管道**：审核(content-review) → 缓存同步(content-change) → 推送(feed-fanout-request)，三个 topic 完全解耦
- Canal 缓存同步管道的设计意图：生产环境用 Canal 监听 MySQL binlog 自动触发，本地开发用 KafkaConsumerService 直接发消息模拟，切换零成本
...
- Redis 写入失败 catch 住，不影响主流程 —— MySQL 是主存储，Redis 是加速缓存，缓存挂了降级读 MySQL
...
**设计决策**：
- **双游标 (publishTime, contentId)**：模拟数据库联合索引 `(publish_time DESC, content_id DESC)` 的效果，解决同时间多条内容的分页跳过/重复问题
- **ZSET member 编码**：contentId 零补到 20 位（`String.format("%020d", contentId)`），如 `00000000000000000042`，保证字典序 = 数值序，ZSET 按字典序排列 member 等价于按 contentId 排序
- **ZSET 游标分页算法**（两步查询，模拟 WHERE score <= cursorScore AND member < cursorMember）：
  - **Step 1**：取 score = cursorScore 且 member < cursor 的成员（同一批次中更早的内容）
  - **Step 2**：如果 Step 1 不够 num 条，继续取 score < cursorScore 的成员（更早的时间段）
  - 两步凑满 num 条，与 MySQL 的 `ORDER BY publish_time DESC, content_id DESC LIMIT n` 语义一致
- content Hash 缓存只存首页展示字段（不含完整内容），Pipeline 批量读取避免 N+1
...
| Redis | 6379 | 缓存层：inbox/outbox ZSET、content Hash、关注列表 ZSET、用户活跃标记。密码 123456 |
| Kafka | 9092 | 三个 topic：`content-review`（审核）→ `content-change`（Canal 缓存同步）→ `feed-fanout-request`（Fanout 推送） |
| MinIO | 9000 | 存图片和视频，bucket 名 buzzfeed，账号 minioadmin |
| FFmpeg | 本地安装 | 视频转码和封面截帧 |
| Canal | 生产环境 | 监听 MySQL binlog，推送 item_info 表变更到 Kafka `content-change` topic（本地未部署，用 Kafka 消息模拟） |
